...
def test_evidence_gap_queue_carries_dossier_plan_and_manual_workbench_fields(tmp_path):
    from source_yield_validation import build_source_yield_validation_report
            ...
            "product_hunt": [
                {
                    ...
                    "missing_evidence": ["stage_funding_or_headcount_missing", "commercial_or_customer_signal_missing"],
                    "action": "research deeper",
                }
            ],
            "x_launches": [],
    ...
    gap = report["evidence_gap_queue"][0]
    manual = report["manual_evidence_queue"][0]
    assert gap["name"] == "AgentFence"
    assert gap["confidence_grade"] == "C"
    assert gap["dossier"]["source_context"]["role"] == "launch_source"
    assert gap["evidence_completion_plan"]
    assert gap["manual_review_checklist"]
    assert gap["promote_if_found"]
    assert gap["discard_if_not_found"]
    assert manual["evidence_completion_plan"] == gap["evidence_completion_plan"]
    assert manual["manual_review_checklist"] == gap["manual_review_checklist"]
