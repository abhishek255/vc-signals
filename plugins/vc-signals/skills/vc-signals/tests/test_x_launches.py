...
def test_run_x_launches_resolves_official_url_embedded_in_x_snippet_without_web_fallback():
    ...
    assert "official_domain_identity_not_confirmed" not in result["launches"][0]["missing_evidence"]
    assert result["launches"][0]["launch_radar_role"] == "launch_signal_not_identity_truth"
    assert result["launches"][0]["identity_resolution_policy"].startswith("X can detect launch")
    assert result["launches"][0]["evidence_completion_plan"]
    assert result["launches"][0]["manual_review_checklist"]
    assert result["launches"][0]["recommended_manual_check"]
    assert "https://xell.pro" in result["launches"][0]["source_outbound_urls"]
...
def test_x_launch_preserves_lower_confidence_chatter_as_watch_when_identity_exists():
    ...
    assert result["launches"][0]["launch_signal"]["confidence_score"] < 60
    assert result["launches"][0]["promote_if"]
    assert result["launches"][0]["discard_if"]
