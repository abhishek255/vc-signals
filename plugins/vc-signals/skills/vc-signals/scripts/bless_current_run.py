...
def _readme_text(manifest: dict, partner_packet: dict, ledger_report: dict) -> str:
    ...
    lines.extend(
        [
            ...
            "## Current Packet",
            "",
            f"- Assign Owner: {packet_summary.get('owner_follow_up', packet_summary.get('assign_owner', 'unknown'))}",
            f"- Partner Review Companies: {packet_summary.get('partner_review_companies', 'unknown')}",
            f"- Review-Worthy Companies: {packet_summary.get('review_worthy_companies', packet_summary.get('review_worthy_research', 'unknown'))}",
            f"- Review-Worthy Market Signals: {packet_summary.get('review_worthy_market_signals', 'unknown')}",
            f"- Evidence Gap Queue: {packet_summary.get('evidence_gap_queue', 'unknown')}",
            f"- Manual Evidence Queue: {packet_summary.get('manual_evidence_queue', 'unknown')}",
            f"- Unsafe promotions: {packet_summary.get('unsafe_promotions', ledger_summary.get('unsafe_promotions', 'unknown'))}",
            "",
            "## Evidence Completion Workflow",
            "",
            "Product Hunt, X, Evidence Gap, and Manual Evidence rows carry source context, evidence completion plans, exact manual checks, promote-if-found, discard-if-not-found, and likely-payoff fields.",
            "",
            "Product Hunt is a launch source, not identity truth. X is launch radar, not identity truth. Official company identity still has to come from official domains, company sites, founder/operator evidence, company pages, or structured/manual metadata.",
        ]
    )
    lines.extend(
        [
