...
def _exact_manual_checks(target: dict) -> list[str]:
    missing = set(target.get("missing_evidence") or [])
    checks = []
    if "official_domain_missing" in missing or "official_domain_identity_not_confirmed" in missing:
        checks.append("Find the official website from the launch page, founder profile, docs, or search result.")
    if "founder_team_missing" in missing or "founder_or_maintainer_missing" in missing:
        checks.append("Confirm founder/team from company about/team page, Product Hunt maker page, GitHub, X, or LinkedIn.")
    if "company_linkedin_missing" in missing or "company_linkedin_or_social_missing" in missing:
        checks.append("Find the company LinkedIn or official company social page for supporting identity confidence.")
    if "stage_or_funding_missing" in missing or "stage_funding_or_headcount_missing" in missing:
        checks.append("Check funding, stage, headcount, jobs, careers, or Crunchbase-style public snippets.")
    if "headcount_missing" in missing:
        checks.append("Check LinkedIn/headcount snippets and company careers/jobs pages.")
    if "commercial_or_customer_signal_missing" in missing:
        checks.append("Check pricing, customers, case studies, docs, deployments, waitlist, or paid plan evidence.")
    if "pricing_docs_or_careers_missing" in missing:
        checks.append("Check pricing/docs/careers pages on the official website.")
    return checks or [target.get("recommended_next_step") or "Do one focused official-site and contradiction check."]


def _manual_guidance(target: dict, unresolved: list[str], hints: dict) -> dict:
    missing = set(target.get("missing_evidence") or [])
    any_hints = any(hints.get(key) for key in hints)
    if unresolved:
        suggested_decision = "still_blocked"
    elif any_hints:
        suggested_decision = "manual_confirmation_needed"
    else:
        suggested_decision = "no_new_public_evidence"
    if "official_domain_missing" in missing or "official_domain_identity_not_confirmed" in missing:
        promote = "Promote if the official domain clearly matches the launch/product and is not a social, repo, article, or directory page."
        discard = "Discard if no official website or durable company identity appears after focused search."
    elif "founder_team_missing" in missing or "founder_or_maintainer_missing" in missing:
        promote = "Promote if founder/operator evidence matches the official company or launch."
        discard = "Discard if no real operator/team evidence appears."
    elif "commercial_or_customer_signal_missing" in missing or "pricing_docs_or_careers_missing" in missing:
        promote = "Promote if pricing, docs, customers, careers, deployment, or paid product proof appears."
        discard = "Discard if the row remains only launch chatter with no product/commercial surface."
    else:
        promote = "Promote if manual review confirms the remaining missing evidence and no contradictions appear."
        discard = "Discard if evidence remains only weak snippets or directories."
    likely_payoff = (
        "High: search found candidate evidence; a human should verify it before promotion."
        if suggested_decision == "manual_confirmation_needed"
        else "Medium: still promising, but blocked until the missing field resolves."
        if suggested_decision == "still_blocked"
        else "Low-medium: public search did not add evidence in this pass."
    )
    return {
        "suggested_decision": suggested_decision,
        "promote_if_found": promote,
        "discard_if_not_found": discard,
        "likely_payoff": likely_payoff,
    }


def _manual_workbench(target: dict, hints: dict, unresolved: list[str]) -> dict:
    checks = _exact_manual_checks(target)
    guidance = _manual_guidance(target, unresolved, hints)
    return {
        "exact_checks": checks,
        "missing_fields": target.get("missing_evidence") or [],
        "evidence_hints_found": {
            key: bool(value)
            for key, value in hints.items()
        },
        "unresolved_gaps": unresolved,
        "recommended_manual_check": target.get("recommended_next_step") or checks[0],
        **guidance,
    }
...
def enrich_targets(
    ...
    *,
    ...
    for target in selected:
        ...
        unresolved = _unresolved_gaps(target, hints)
        workbench = _manual_workbench(target, hints, unresolved)
        rows.append(
            {
                ...
                "unresolved_gaps": unresolved,
                "manual_workbench": workbench,
                "recommended_manual_check": workbench["recommended_manual_check"],
                "promote_if": workbench["promote_if_found"],
                "discard_if": workbench["discard_if_not_found"],
                "likely_payoff": workbench["likely_payoff"],
                "promote_if_found": workbench["promote_if_found"],
                "discard_if_not_found": workbench["discard_if_not_found"],
